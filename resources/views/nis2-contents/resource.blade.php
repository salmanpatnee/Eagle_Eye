...
@section('title', 'NIS2 Content Resource')
@section('content')
    <div>
        <x-table.action-wrapper title="{{ $nis2Content?->id ? 'Update' : 'New' }} Resource">
            <x-action.button label="View" route_name="nis2-contents.index" />
        </x-table.action-wrapper>

        <form action="{{ route('nis2-contents.index') }}">
            @csrf
            <input type="hidden" id="resourceable_id" value="{{ $nis2Content->id }}">
            <input type="hidden" id="resourceable_type" value="App\Models\Nis2Content">
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6">
                <x-form.grid-col>
                    <div>
                        <x-form.field label="Content Title" name="title" required="true"
                            placeholder="Enter Content Title" :value="$nis2Content?->title" readonly />
                    </div>
                </x-form.grid-col>
                ...
                <div class="flex justify-end">
                    <x-form.submit label="Section" :isUpdate="$nis2Content?->id" />
                </div>
            </div>
        </form>
    ...
    <script>
        ...
        function initFilePondUploader(inputId, resourceType, acceptedFileTypes = null) {
            const options = {
                server: {
                    process: {
                        url: '{{ route('nis2-contents.resource.store') }}',
                        method: 'POST',
                        headers: {
