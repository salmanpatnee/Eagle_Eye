    ...
    <div class="relative bg-gradient-to-br from-brand-900 to-brand-950 dark:from-brand-950 dark:to-gray-900 border border-brand-800/30 dark:border-gray-700/50 px-6 py-10 rounded-3xl h-full min-h-[260px] flex flex-col overflow-hidden transition-all duration-500 hover:shadow-2xl hover:shadow-brand-500/20 hover:border-brand-700 hover:scale-[1.02] group-hover:from-brand-800 group-hover:to-brand-950">
        ...
        <div class="relative z-10 flex items-start justify-center flex-grow">
            <div class="text-center px-2">
                ...
                @endif
                <h4 class="min-h-[80px] font-bold text-2xl leading-relaxed text-white group-hover:text-white transition-colors {{ isset($title_ar) && !empty($title_ar) ? 'mt-2' : '' }}">
                    {{ $title }}
                </h4>

                <!-- Article Count Badge -->
                <div class="mt-6 inline-flex items-center gap-2 px-4 py-2 bg-brand-500/20 backdrop-blur-sm border border-brand-400/30 rounded-full group-hover:bg-brand-500/30 transition-colors duration-300">
                    ...
                    <span class="text-sm font-semibold text-white">{{ $item->processes_count ?? 0 }} {{ $item->processes_count == 1 ? 'Article' : 'Articles' }}</span>
                </div>
