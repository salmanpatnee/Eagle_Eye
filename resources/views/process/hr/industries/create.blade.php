    ...
    <div>
        ...
        <form action="{{ isset($industry) ? route('industries.update', $industry->id) : route('industries.store') }}" method="POST">
            ...
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6">
                <x-form.grid-col>
                    <div>
                        <x-form.field label="Industry ID" label_ar="رمز الصناعة" name="industry_id" required="true" :readonly="$industry?->industry_id"
                            placeholder="Enter Industry ID" :value="$industry?->industry_id" />
                    </div>
                    <div>
                        <x-form.field label="Industry Name" label_ar="اسم الصناعة" name="industry_name" required="true"
                            placeholder="Enter Industry Name" :value="$industry?->industry_name" />
                    </div>
                </x-form.grid-col>
                <x-form.grid-col>
                    <div>
                        <x-form.field label="Sector" label_ar="القطاع" name="sector" 
                ...
                <div class="flex justify-end">
                    <x-form.submit label="Industry" label_ar="الصناعة" :isUpdate="$industry?->industry_id" />
                </div>
            </div>
        </form>
