...
<body>
    ...
    <section id="sidebar">
        ...
        @include('process/KpiCategories/sidebar')
    </section>
    <!-- SIDEBAR -->
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('kpi-categories.delete') }}">
            @csrf
            @method('DELETE')
            <div class="TableHeading">
                <div class="PageHead">
                    <p class="PageHeadArbTxt"> مؤشرات الأداء الرئيسية</p>
                    <p class="PageHeadEngTxt">KPI Categories</p>
                </div>
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز مؤشرات الأداء الرئيسية</p>
                            ...
                            <tr>
                                <td>
                                    <input type="radio" name="record" class="record"
                                        value="{{ $category->$primaryKey }}" required>
                                </td>
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a href="/kpi-categories/{{ $category->kpi_id }}">{{ $category->kpi_id }}</a>
                                </td>
                                <td>{{ $category->kpi_name }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/kpi-categories/edit/${selectedRadio.value}`;
            } else {
