...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('kpi-standards.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
            <div class="TableHeading">
                <div class="PageHead">
                    <p class="PageHeadArbTxt">معيار مؤشرات الأداء الرئيسية</p>
                    <p class="PageHeadEngTxt">KPI Standards</p>
                </div>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/kpi-standards/edit/${selectedRadio.value}`;
            } else {
