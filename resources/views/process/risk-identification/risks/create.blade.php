        ...
        <form action="{{ isset($risk) ? route('risks.update', $risk->id) : route('risks.store') }}" method="POST">
            ...
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6">
                ...
                <x-form.textarea-field label="Risk Profile" label_ar="تفاصيل المخاطر" name="risk_profile"
                    ...
                    <x-form.textarea-field label="Risk Consequences" label_ar="آثار المخاطر" name="risk_consequences"
                    placeholder="Enter Risk Consequences" :value="$risk?->risk_consequences" />
                ...
                <x-form.grid-col>
                    ...
                    <div>
                        <x-form.multiselect label="Controls" required="true" label_ar="وكيل الضوابط"
                            name="controls[]" :value="$controlIds" :show_key="true" :data="$controls" id_key="control_id"
                ...
                <x-form.grid-col>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Critical Assets?"
                            ...
                            :value="old('risk_critical_asset', $risk?->risk_critical_asset ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Cloud?"
                            label_ar="المخاطر المرتبطة حصريًا بالسحابة؟" name="risk_cloud" :value="old('risk_cloud', $risk?->risk_cloud ?? 'No')"
                ...
                <x-form.grid-col>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Telework?"
                            label_ar="المخاطر مرتبطة حصريًا بالعمل عن بعد؟" name="risk_telework" :value="old('risk_telework', $risk?->risk_telework ?? 'No')"
                            :custom_data="$yesNoOptions" />
                    </div>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Social Media?"
                            ...
                            :value="old('risk_social_media', $risk?->risk_social_media ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                </x-form.grid-col>
                ...
                <x-form.grid-col>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Data Privacy?"
                            ...
                            :value="old('risk_data_privicy', $risk?->risk_data_privicy ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                    <div>
                        <x-form.select label="Risk Exclusively Related to PII?"
                            ...
                            :value="old('risk_pii', $risk?->risk_pii ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                </x-form.grid-col>
                ...
                <x-form.grid-col>
                    <div>
                        <x-form.select label="Risk Exclusively Related to PCI/DSS?"
                            label_ar="؟PCI/DSS المخاطر المرتبطة حصريًا" name="risk_pci_dss" :value="old('risk_pci_dss', $risk?->risk_pci_dss ?? 'No')"
                            :custom_data="$yesNoOptions" />
                    </div>
                    <div>
                        <x-form.select label="Risk Exclusively Related to E-Commerce?"
                            ...
                            :value="old('risk_e_commerce', $risk?->risk_e_commerce ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                </x-form.grid-col>
                ...
                <x-form.grid-col>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Infrastructure?"
                            ...
                            :value="old('risk_infrastructure', $risk?->risk_infrastructure ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Application?"
                            label_ar="المخاطر المرتبطة حصرا بالتطبيق؟" name="risk_application" :value="old('risk_application', $risk?->risk_application ?? 'No')"
                            :custom_data="$yesNoOptions" />
                    </div>
                </x-form.grid-col>
                ...
                <x-form.grid-col>
                    <div>
                        <x-form.select label="Risk Exclusively Related to HR?"
                            label_ar="المخاطر المتعلقة حصرا بالموارد البشرية؟" name="risk_hr" :value="old('risk_hr', $risk?->risk_hr ?? 'No')"
                            :custom_data="$yesNoOptions" />
                    </div>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Physical Security?"
                            ...
                            :value="old('risk_physical_security', $risk?->risk_physical_security ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                </x-form.grid-col>
                ...
                <x-form.grid-col>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Third Party?"
                            label_ar="المخاطر المرتبطة حصرا بطرف خارجي؟" name="risk_third_party" :value="old('risk_third_party', $risk?->risk_third_party ?? 'No')"
                            :custom_data="$yesNoOptions" />
                    </div>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Operational Technology?"
                            ...
                            :value="old('risk_operational', $risk?->risk_operational ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                </x-form.grid-col>
                ...
                <x-form.grid-col>
                    <div>
                        <x-form.select label="Risk Exclusively Related to Payments?"
                            label_ar="المخاطر المرتبطة حصرا بالمدفوعات؟" name="risk_payment" :value="old('risk_payment', $risk?->risk_payment ?? 'No')"
                            :custom_data="$yesNoOptions" />
                    </div>
                    <div>
                        <x-form.select label="Risk Exclusively Related to E-Banking?"
                            ...
                            :value="old('risk_e_banking', $risk?->risk_e_banking ?? 'No')" :custom_data="$yesNoOptions" />
                    </div>
                </x-form.grid-col>
