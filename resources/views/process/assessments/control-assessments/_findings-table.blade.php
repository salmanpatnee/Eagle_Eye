     ...
     <x-slot:body>
         @forelse ($paginatedFindings as $finding)
             <tr class="hover:bg-gray-50 dark:hover:bg-white/[0.03]">
                 ...
                 <x-table.td>{{ $finding->control_finding_id }}</x-table.td>
                 <x-table.td>{{ $finding->control_finding_name }}</x-table.td>
                 <x-table.td>{{ $finding->control_implementation_status }}</x-table.td>
                 <x-table.td action_col="true">
                     ...
                     <x-action.delete route_name="control-assessment-findings.destroy" param="{{ $finding->id }}" />
                 </x-table.td>
             </tr>
         @empty
             <tr>
