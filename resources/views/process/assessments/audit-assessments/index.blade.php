    ...
    <div>
        ...
        <form action="{{ route('audit-assessments.index') }}" method="GET">
            <div class="space-y-6 border-t border-gray-100 p-2 sm:p-6">
                <x-form.grid-3-col>
                    <div>
                        <x-form.select label="Audits" label_ar=" الضوابط" name="control_id" :value="$auditId" :data="$auditNames"
                            id_key="audit_id" value_key="name" onchange="this.form.submit()" hide_keys="true" />
                    </div>
                    <div>
                        <x-form.select label="Controls" label_ar="مراجعة" name="control_id" :value="$controlId"
