...
<body>
    ...
    <section id="sidebar">
        ...
        @include('process/objectives/sidebar')
    </section>
    <!-- SIDEBAR -->
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('objectives.destroy') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز أهداف</p>
                                <p class="ListHeadEngTxt">Objective ID</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt"> أهداف</p>
                                <p class="ListHeadEngTxt">Objective</p>
                        </tr>
                        @foreach ($objectives as $objective)
                            <tr>
                                <td>
                                    <input type="radio" name="record" class="record" value="{{ $objective->id }}"
                                        required>
                                </td>
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a
                                        href="{{ route('objectives.show', $objective->id) }}">{{ $objective->objective_id }}</a>
                                </td>
    ...
    <script>
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/objectives/${selectedRadio.value}/edit`;
            } else {
