    ...
    <main>
        <header class="report-header" style="margin-bottom: 30px;">
            @if ($organizationData)
                ...
                <h2 style="font-weight: bold; font-size: 18px; font-family: 'Roboto'" class="mt-0">Control vs Audit
                </h2>
                    ...
                    @forelse ($controlsWithAuditFindings as $row)
                        <tr style="border: 1px solid #000;">
                            <td style="border: 1px solid #000; padding: 12px; font-size: 12px; text-align: left;">
                                {{ $loop->index + 1 }}</td>
                            <td style="border: 1px solid #000; padding: 12px; font-size: 12px; text-align: left;">
                                <a href="{{ route('controls.show', $row->id) }}" style="text-decoration: none;">
                                    {{ $row->control_id }}
                                </a>
                            </td>
                            <td style="border: 1px solid #000; padding: 12px; font-size: 12px; text-align: left;">
                            ...
                            <td style="border: 1px solid #000; padding: 12px; font-size: 12px; text-align: left;">
                                @foreach ($row->findings as $finding)
                                    <a href="{{ route('audit-findings.show', $finding->id) }}"
