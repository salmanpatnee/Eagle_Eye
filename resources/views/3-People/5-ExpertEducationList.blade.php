...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('delete.education') }}">
            @csrf
            @method('DELETE')
            <div class="TableHeading">
                <h1>Expert's Education</h1>
                <div class="ButtonContainer">
                    <a href="" class="DisabledButton">View</a>
                    <a href="/expert-education-input" class="MoreButton">Add</a>
            ...
            <div class="ListTable">
                <table cellspacing="0">
                    <tr>
                        ...
                        <th style="padding-right: 0px;">Education ID</th>
                        <th style="padding-right: 100px;">Education Name</th>
                        <th style="padding-right: 0px;">University Name</th>
                    </tr>
                    @foreach ($columns as $data)
                        <tr>
                            <td><input type="checkbox" name="selecteddelete[]" value="{{ $data->education_id }}">
                            </td>
                            <td><a
                                    href="/expert-education-table/{{ $data->education_id }}">{{ $data->education_id }}</a>
                            </td>
                            <td>{{ $data->education_name }}</td>
                            <td>{{ $data->education_university }}</td>
                        </tr>
                    @endforeach
                </table>
            </div>
