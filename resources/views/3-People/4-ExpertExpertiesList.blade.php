...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('delete.expertise') }}">
            @csrf
            @method('DELETE')
            ...
            <div class="ListTable">
                <table cellspacing="0">
                    <tr>
                        ...
                        <th style="padding-right: 0px;">Expertise ID</th>
                        <th style="padding-right: 100px;">Expertise Name</th>
                    </tr>
                    @foreach ($columns as $data)
                        <tr>
                            <td><input type="checkbox" name="selecteddelete[]" value="{{ $data->expert_experties_id }}">
                            </td>
                            <td><a
                                    href="/expert-expertise-table/{{ $data->expert_experties_id }}">{{ $data->expert_experties_id }}</a>
                            </td>
                            <td>{{ $data->expert_experties_name }}</td>
                        </tr>
                    @endforeach
                </table>
            </div>
