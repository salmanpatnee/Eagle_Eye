...
@endpush
@section('title', $nis2ContentWithVideos->title . ' Resource: Videos')
@section('content')

    <div class="gap-6 grid grid-cols-1 px-4">
        <div class="hover:shadow-lg mx-auto rounded-lg shadow text-white transition">
            <x-iso-content-card title="{{ $nis2ContentWithVideos->title }}">
                {{ $nis2ContentWithVideos->description }}
            </x-iso-content-card>
        </div>
    </div>
...
@php
    $videoCount = $nis2ContentWithVideos->resources->count();
@endphp
...
@section('additional_content')
    <div class="bg-white my-6 p-5 rounded-2xl">
        <header class="text-center bg-brand-950 font-bold  mb-3 p-3 rounded-md text-white">
            <h1>Video Explanations of {{ $nis2ContentWithVideos->title }}</h1>
        </header>

        <div class="process-content">
            @if ($videoCount === 1)
                <div class="grid grid-cols-1 gap-6">
                    @foreach ($nis2ContentWithVideos->resources as $video)
                        <div>
