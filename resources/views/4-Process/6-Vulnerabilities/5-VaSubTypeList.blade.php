...
<body>
    ...
    <div class="wrapper">
        ...
        <!-- CONTENT -->
        <div class="IndiTable">
            <form method="POST" action="{{ route('vasubtype.delete') }}" id="deleteForm">
                @csrf
                @method('DELETE')
                    ...
                    <div class="ListTable">
                        <table cellspacing="0">
                            <tr class="table-header">
                                ...
                                <th style="padding-right: 0px;">
                                    <p class="ListHeadArbTxt">رمز النوع الفرعي للضعف</p>
                                    <p class="ListHeadEngTxt">VA Sub-Type ID</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">اسم النوع الفرعي للضعف</p>
                                    <p class="ListHeadEngTxt">VA Sub-Type Name</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">اسم النوع للضعف</p>
                                    <p class="ListHeadEngTxt">VA Type Name</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">وصف النوع الفرعي للضعف</p>
                                    <p class="ListHeadEngTxt">VA Sub-Type Description</p>
                                </th>
                            </tr>
                            @foreach ($columns as $data)
                                <tr>
                                    ...
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td><a
                                            href="/va-sub-type-table/{{ $data->va_sub_type_id }}">{{ $data->va_sub_type_id }}</a>
                                    </td>
                                    <td>{{ $data->va_sub_type_name }}</td>
                                    <td>{{ $data->type?->va_type_name }}</td>
                                    <td>{{ $data->va_sub_type_description }}</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/va-sub-type/edit/${selectedRadio.value}`;
            } else {
