...
<body>
    ...
    <div class="wrapper">
        ...
        <!-- CONTENT -->
        <div class="IndiTable">
            <form method="POST" action="{{ route('va.delete') }}" id="deleteForm">
                @csrf
                @method('DELETE')
                    ...
                    <div class="ListTable">
                        <table cellspacing="0">
                            <tr class="table-header">
                                ...
                                <th style="padding-right: 0px;">
                                    <p class="ListHeadArbTxt">رمز الضعف</p>
                                    <p class="ListHeadEngTxt">VA ID</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">اسم الضعف</p>
                                    <p class="ListHeadEngTxt">VA Name</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">نوع الضعف</p>
                                    <p class="ListHeadEngTxt">VA Type</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">النوع الفرعي للضعف
    
                                    </p>
                                    <p class="ListHeadEngTxt">VA Subtype</p>
                                </th>
                            </tr>
                            @foreach ($vanames as $vaname)
                                <tr>
                                    <td>
                                        <input type="radio" name="record" class="record"
                                            value="{{ $vaname->$primaryKey }}" required>
                                    </td>
                                    <td>{{ $loop->index + 1 }}</td>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/va/edit/${selectedRadio.value}`;
            } else {
