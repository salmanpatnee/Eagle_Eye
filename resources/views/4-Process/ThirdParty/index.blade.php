...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('third-party.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز الطرف الثالث</p>
                                <p class="ListHeadEngTxt">Third Party ID</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">اسم الطرف الثالث</p>
                                <p class="ListHeadEngTxt">Third Party Name</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">بلد المنشأ</p>
                                <p class="ListHeadEngTxt">Country Origin</p>
                            </th>
                        </tr>
                            ...
                            <tr>
                                <td>
                                    <input type="radio" name="record" class="record"
                                        value="{{ $thirdParty->$primaryKey }}" required>
                                </td>
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a
                                        href="{{ route('third-party.show', $thirdParty->tpt_id) }}">{{ $thirdParty->tpt_id }}</a>
                                </td>
                                <td>{{ $thirdParty->tpt_name }}</td>
                                <td>{{ $thirdParty->tpt_country_origin }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    if (selectedRadio) {
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/third-party/edit/${selectedRadio.value}`;
            } else {
