...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('patch.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز تصحيح</p>
                                <p class="ListHeadEngTxt">Patch ID</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">اسم تصحيح</p>
                                <p class="ListHeadEngTxt">Patch Name</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">اسم الطرف الثالث</p>
                                <p class="ListHeadEngTxt">Third Party</p>
                            </th>
                        </tr>
                            ...
                            <tr>
                                <td>
                                    <input type="radio" name="record" class="record"
                                        value="{{ $patch->$primaryKey }}" required>
                                </td>
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a href="/patch/{{ $patch->patch_id }}">{{ $patch->patch_id }}</a>
                                </td>
                                <td>{{ $patch->patch_name }}</td>
                                <td>{{ $patch->thirdParty->tpt_name }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    if (selectedRadio) {
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/patch/edit/${selectedRadio.value}`;
            } else {
