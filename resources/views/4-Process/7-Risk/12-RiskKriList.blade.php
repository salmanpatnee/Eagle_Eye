...
<body>
    ...
    <!-- SIDEBAR -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('riskkri.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز مؤشر المخاطر الرئيسية</p>
                                <p class="ListHeadEngTxt">KRI ID</p>
                            </th>
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">اسم مؤشر المخاطر الرئيسية</p>
                                <p class="ListHeadEngTxt">KRI Name</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">قيمة مؤشر المخاطر الرئيسي</p>
                                <p class="ListHeadEngTxt">KRI Value</p>
                            </th>
                        </tr>
                        @foreach ($columns as $data)
                            <tr>
                                ...
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a
                                        href="/kri-table/{{ $data->key_risk_indicator_id }}">{{ $data->key_risk_indicator_id }}</a>
                                </td>
                                <td><a
                                        href="/kri-table/{{ $data->key_risk_indicator_id }}">{{ $data->key_risk_indicator_name }}</a>
                                </td>
                                <td>{{ $data->key_risk_indicator_value }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/kri/edit/${selectedRadio.value}`;
            } else {
