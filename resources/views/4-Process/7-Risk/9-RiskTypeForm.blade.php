    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function generateID() {
                ...
                var inputElement = document.getElementById('RiskTypeId');
                ...
                var generatedID = 'RSK-TYP-' + formattedNumber;
...
<body>
    ...
    <div class="IndiTable">
        ...
        <form id="form"
            action="{{ isset($risktype) ? route('risktype.update', $risktype->id) : route('risktype.store') }}"
            method="POST">
            @csrf
            @if (isset($risktype))
                @method('PUT')
                <input type="hidden" name="id" value="{{ $risktype->id }}">
            @endif
            <table cellspacing="0">
                <div class="ContentTableSection">
                    <div class="ContentTable">
                        <div class="column">
                            ...
                            <p><input type="text" name="risk_type_id" id="risk_type_id" class="sh-tx"
                                    placeholder="Enter ID" value="{{ old('risk_type_id', $risktype?->risk_type_id) }}"
                                    {{ $risktype?->risk_type_id ? 'readonly' : '' }} required>
                                @error('risk_type_id')
                        ...
                        <div class="column">
                            ...
                            <p><input type="text" name="risk_type_name" id="risk_type_name" class="sh-tx"
                                    placeholder="Write Name"
                                    value="{{ old('risk_type_name', $risktype?->risk_type_name) }}" required>
                                @error('risk_type_name')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </p>
                        </div>
                    ...
                    <div class="ContentTablebg">
                        <div class="column">
                            ...
                            <p><input type="text" name="risk_type_description" id="risk_type_description"
                                    class="bg-tx" placeholder="Write Description"
                                    value="{{ old('risk_type_description', $risktype?->risk_type_description) }}">
                                @error('risk_type_description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </p>
                        </div>
