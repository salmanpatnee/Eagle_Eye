    ...
    <body>
        ...
        <!-- SIDEBAR -->
        <div class="IndiTable">
            <form method="POST" action="{{ route('risktreatment.delete') }}" id="deleteForm">
                @csrf
                @method('DELETE')
                    ...
                    <div class="ListTable">
                        <table cellspacing="0">
                            <tr class="table-header">
                                ...
                                <th style="padding-right: 0px;">
                                    <p class="ListHeadArbTxt">رمز خيارات علاج المخاطر</p>
                                    <p class="ListHeadEngTxt">Risk Treatment ID</p>
                                </th>
                                <th style="padding-right: 0px;">
                                    <p class="ListHeadArbTxt">اسم خيارات علاج المخاطر</p>
                                    <p class="ListHeadEngTxt">Risk Treatment Name</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">وصف خيارات علاج المخاطر</p>
                                    <p class="ListHeadEngTxt">Risk Treatment Description</p>
                                </th>
                            </tr>
                            @foreach ($columns as $data)
                                <tr>
                                    ...
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td><a
                                            href="/risk-treatment-option-table/{{ $data->risk_treatment_id }}">{{ $data->risk_treatment_id }}</a>
                                    </td>
                                    <td>{{ $data->risk_treatment_name }}</td>
                                    <td>{{ $data->risk_treatment_description }}</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
    ...
    if (selectedRadio) {
        ...
        alert('Please select a record to delete.');
            ...
            document.getElementById('btnUpdate').addEventListener('click', function(event) {
                ...
                if (selectedRadio) {
                    window.location.href = `/risk-treatment-option/edit/${selectedRadio.value}`;
                } else {
