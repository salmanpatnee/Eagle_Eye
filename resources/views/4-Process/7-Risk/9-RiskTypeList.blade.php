...
<body>
    ...
    <!-- SIDEBAR -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('risktype.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز نوع المخاطرة</p>
                                <p class="ListHeadEngTxt">Risk Type ID</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">اسم نوع المخاطرة</p>
                                <p class="ListHeadEngTxt">Risk Type Name</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">وصف نوع المخاطرة</p>
                        ...
                        @foreach ($columns as $data)
                            <tr>
                                ...
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a href="/risk-type-table/{{ $data->risk_type_id }}">{{ $data->risk_type_id }}</a>
                                </td>
                                <td>{{ $data->risk_type_name }}</td>
                                <td>{{ $data->risk_type_description }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    if (selectedRadio) {
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/risk-type/edit/${selectedRadio.value}`;
            } else {
