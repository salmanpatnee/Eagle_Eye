    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            function generateID() {
                ...
                var inputElement = document.getElementById('RiskGroupId');
                ...
                var generatedID = 'RSK-GRP-' + formattedNumber;
...
<body>
    ...
    <div class="IndiTable">
        ...
        <div class="TableHeading">
            <div class="PageHead">
                <p class="PageHeadArbTxt">مجموعة المخاطر</p>
                <p class="PageHeadEngTxt">Risk Group</p>
            </div>
            <div class="ButtonContainer">
                <a href="/risk-group-list" class="MoreButton">
        ...
        <form id="form" action="{{ isset($riskgroup) ? route('riskgroup.update', $riskgroup->id) : route('riskgroup.store') }}"
            method="POST">
            @csrf
            @if (isset($riskgroup))
                @method('PUT')
                <input type="hidden" name="id" value="{{ $riskgroup->id }}">
            @endif
            <table cellspacing="0">
                <div class="ContentTableSection">
                    <div class="ContentTable">
                        <div class="column">
                            <div class="FieldHead">
                                <p class="FieldHeadEngTxt">Risk Group ID</p>
                                <p class="FieldHeadArbTxt">رمز مجموعة المخاطر</p>
                            </div>
                            <p><input type="text" name="risk_group_id" id="risk_group_id" class="sh-tx"
                                    placeholder="Enter ID"
                                    value="{{ old('risk_group_id', $riskgroup?->risk_group_id) }}"
                                    {{ $riskgroup?->risk_group_id ? 'readonly' : '' }} required>
                                @error('risk_group_id')
                        ...
                        <div class="column">
                            ...
                            <p><input type="text" name="risk_group_name" id="risk_group_name" class="sh-tx"
                                    placeholder="Write Name"
                                    value="{{ old('risk_group_name', $riskgroup?->risk_group_name) }}" required>
                                @error('risk_group_name')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </p>
                        </div>
                    ...
                    <div class="ContentTablebg">
                        <div class="column">
                            <div class="FieldHead">
                                <p class="FieldHeadEngTxt">Risk Group Description</p>
                                <p class="FieldHeadArbTxt">وصف مجموعة المخاطر</p>
                            </div>
                            <p><input type="text" name="risk_group_description" id="risk_group_description"
                                    class="bg-tx" placeholder="Write Description"
                                    value="{{ old('risk_group_description', $riskgroup?->risk_group_description) }}">
                                @error('risk_group_description')
                                    <span class="text-danger">{{ $message }}</span>
                                @enderror
                            </p>
                        </div>
                    ...
                    <div class="ContentTable">
                        <div class="column">
                            <div class="FieldHead">
                                <p class="FieldHeadEngTxt">Risk Group Owner Name</p>
                                <p class="FieldHeadArbTxt">اسم صاحب مجموعة المخاطر</p>
                            </div>
                            <p>
                                <select name="owner_id" id="owner_id" class="sh-tx">
                                    <option value="" disabled selected>Select Option</option>
                                    @foreach ($ownername as $row)
                                        <option value="{{ $row->owner_role_id }}"
                                            {{ $row->owner_role_id == old('owner_id', $riskgroup?->owner_id) ? 'selected' : '' }}>
