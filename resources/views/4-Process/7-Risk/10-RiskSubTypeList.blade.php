...
<body>
    ...
    <!-- SIDEBAR -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('risksubtype.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز النوع الفرعي للمخاطر</p>
                                <p class="ListHeadEngTxt">Risk Sub-Type ID</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">اسم النوع الفرعي للمخاطر</p>
                                <p class="ListHeadEngTxt">Risk Sub-Type Name!</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">وصف النوع الفرعي للمخاطر</p>
                        ...
                        @foreach ($columns as $data)
                            <tr>
                                <td>
                                    <input type="radio" name="record" class="record" value="{{ $data->$primaryKey }}" required>
                                </td>
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a
                                        href="/risk-sub-type-table/{{ $data->risk_sub_type_id }}">{{ $data->risk_sub_type_id }}</a>
                                </td>
                                <td>{{ $data->risk_sub_type_name }}</td>
                                <td>{{ $data->risk_sub_type_description }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/risk-sub-type/edit/${selectedRadio.value}`;
            } else {
