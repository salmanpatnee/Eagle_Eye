...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('risk-acceptance.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز قبول المخاطر</p>
                                <p class="ListHeadEngTxt">ID</p>
                            </th>
                        ...
                        @foreach ($riskAcceptances as $data)
                            <tr>
                                <td>
                                ...
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a
                                        href="/risk-acceptance-table/{{ $data->risk_acceptance_id }}">{{ $data->risk_acceptance_id }}</a>
                                </td>
                                <td>{{ $data->risk_acceptance_start_date }}</td>
                                <td>{{ $data->risk_acceptance_end_date }}</td>
                                <td>{{ $data->control?->control_name }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/risk-acceptance/edit/${selectedRadio.value}`;
            } else {
