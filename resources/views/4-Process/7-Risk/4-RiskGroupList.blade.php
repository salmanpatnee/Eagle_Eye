...
<body>
    ...
    <!-- SIDEBAR -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('riskgroup.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز مجموعة المخاطر</p>
                                <p class="ListHeadEngTxt">Risk Group ID</p>
                            </th>
                        ...
                        @foreach ($columns as $data)
                            <tr>
                                ...
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a
                                        href="/risk-group-table/{{ $data->risk_group_id }}">{{ $data->risk_group_id }}</a>
                                </td>
                                <td>{{ $data->risk_group_name }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    <script>
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/risk-group/edit/${selectedRadio.value}`;
            } else {
