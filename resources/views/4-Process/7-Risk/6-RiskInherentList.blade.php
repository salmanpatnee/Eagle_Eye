...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('RiskInherent.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز المخاطر الكامنة</p>
                                <p class="ListHeadEngTxt">Risk Inherent ID</p>
                            </th>
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">درجة المخاطرة</p>
                                <p class="ListHeadEngTxt">Risk Score</p>
                            </th>
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">وصف المخاطر الكامنة</p>
                        ...
                        @foreach ($columns as $data)
                            <tr>
                                ...
                                <td style="width: 150px;">{{ $loop->index + 1 }}</td>
                                <td style="width: 150px;"><a
                                        href="/risk-inherent-table/{{ $data->risk_inherent_id }}">{{ $data->risk_inherent_id }}</a>
                                </td>
                                <td style="width: 150px;">{{ $data->risk_inherent_score }}</td>
                                <td>{{ $data->risk_inherent_description }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    if (selectedRadio) {
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/risk-inherent/edit/${selectedRadio.value}`;
            } else {
