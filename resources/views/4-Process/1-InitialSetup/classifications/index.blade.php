...
<body>
                    ...
                    <div class="ListTable">
                        <table cellspacing="0">
                            <tr class="table-header">
                                ...
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">مصدر التصنيف
    
                                    </p>
                                    <p class="ListHeadEngTxt">Classification Source</p>
                                </th>
                            </tr>
                                ...
                                <tr>
                                    <td>
                                        <input type="radio" name="record" class="record"
                                            value="{{ $classification->id }}" required>
                                    </td>
                                    </td>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td><a
                                            href="{{ route('classifications.show', $classification->id) }}">{{ $classification->classification_id }}</a>
                                    </td>
                                    <td>{{ $classification->classification_name }}</td>
                                    <td>{{ $classification->classification_source }}</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
    ...
    <script>
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/classifications/edit/${selectedRadio.value}`;
            } else {
