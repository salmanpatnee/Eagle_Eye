...
<body>
            ...
            <form method="POST" action="{{ route('departments.destroy') }}" id="deleteForm">
                ...
                @if (session('error'))
                    <div class="alert-danger">
                        Department  cannot be deleted due to existing dependencies.
                    </div>
                @endif
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز القسم</p>
                                <p class="ListHeadEngTxt">Department IDs</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">اسم القسم</p>
                        ...
                        @foreach ($departments as $department)
                            <tr>
                                <td>
                                    <input type="radio" name="record" class="record" value="{{ $department->id }}"
                                        required>
                                </td>
                                <td>{{ $loop->index + 1}}</td>
                                <td><a
                                        href="{{ route('departments.show', $department->id) }}">{{ $department->department_id }}</a>
                                </td>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/departments/edit/${selectedRadio.value}`;
            } else {
