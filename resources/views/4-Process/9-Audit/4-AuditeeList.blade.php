...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('auditees.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز مراجعة</p>
                                <p class="ListHeadEngTxt">Auditee ID</p>
                            </th>

                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">مراجع الاسم</p>
                                <p class="ListHeadEngTxt">Auditee Name</p>
                            </th>
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">مراجع القسم</p>
                                <p class="ListHeadEngTxt">Auditee Department</p>
                            </th>
                        </tr>
                        @foreach ($columns as $data)
                            <tr>
                                ...
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a href="/auditee-table/{{ $data->auditee_id }}">{{ $data->auditee_id }}</a>
                                </td>
                                <td>{{ $data->auditee_first_name }} {{ $data->auditee_last_name }}</td>
                                <td>{{ $data->auditee_department }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    <script>
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/auditee/${selectedRadio.value}/edit`;
            } else {
