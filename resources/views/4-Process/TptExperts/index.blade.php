...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('tpt-experts.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                ...
                <div class="ListTable">
                    <table cellspacing="0">
                        <tr class="table-header">
                            ...
                            <th style="padding-right: 0px;">
                                <p class="ListHeadArbTxt">رمز خبير الطرف الثالث</p>
                                <p class="ListHeadEngTxt">TPT Expert ID</p>
                            </th>
                            <th style="padding-right: 100px;">
                                <p class="ListHeadArbTxt">اسم خبير الطرف الثالث</p>
                                <p class="ListHeadEngTxt">TPT Expert Name</p>
                        </tr>
                        @foreach ($tptExperties as $tptExpert)
                            <tr>
                                <td>
                                    <input type="radio" name="record" class="record" value="{{ $tptExpert->$primaryKey }}"
                                        required>
                                </td>
                                <td>{{ $loop->index + 1 }}</td>
                                <td><a
                                        href="/tpt-experts/{{ $tptExpert->tpt_experties_id }}">{{ $tptExpert->tpt_experties_id }}</a>
                                </td>
                                <td>{{ $tptExpert->tpt_experties_name }}</td>
                            </tr>
                        @endforeach
                    </table>
                </div>
    ...
    if (selectedRadio) {
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/tpt-experts/edit/${selectedRadio.value}`;
            } else {
