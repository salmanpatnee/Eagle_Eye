...
<body>
    ...
    <div class="wrapper">
        ...
        <!-- CONTENT -->
        <div class="IndiTable">
            <form method="POST" action="{{ route('threattype.delete') }}" id="deleteForm">
                @csrf
                @method('DELETE')
                        ...
                        <table cellspacing="0">
                            <tr class="table-header">
                                ...
                                <th style="padding-right: 0px;">
                                    ...
                                    <p class="ListHeadEngTxt">Threat Agent Type ID</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">اسم نوع وكيل التهديد</p>
                                    <p class="ListHeadEngTxt">Threat Agent Type Name</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">وصف نوع وكيل التهديد</p>
                                    <p class="ListHeadEngTxt">Threat Agent Type Description</p>
                                </th>
                            </tr>
                                ...
                                <tr>
                                    <td>
                                        <input type="radio" name="record" class="record"
                                            value="{{ $threatAgentType->$primaryKey }}" required>
                                    </td>
                                    <td>{{ $loop->index + 1 }}</td>
                                    <td style="width: 180px;"><a
                                            href="/threat-agent-type-table/{{ $threatAgentType->threat_agent_type_id }}">{{ $threatAgentType->threat_agent_type_id }}</a>
                                    </td>
                                    <td>{{ $threatAgentType->threat_agent_type_name }}</td>
                                    <td>{{ $threatAgentType->threat_agent_type_description }}</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/threat-agent-type/edit/${selectedRadio.value}`;
            } else {
