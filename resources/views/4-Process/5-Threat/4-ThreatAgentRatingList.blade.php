...
<body>
    ...
    <div class="wrapper">
        ...
        <!-- CONTENT -->
        <div class="IndiTable">
            <form method="POST" action="{{ route('threatrating.delete') }}" id="deleteForm">
                @csrf
                @method('DELETE')
                    ...
                    <div class="ListTable">
                        <table cellspacing="0">
                            <tr class="table-header">
                                ...
                                <th style="padding-right: 0px;">
                                    <p class="ListHeadArbTxt">رمز نقاط وكيل التهديد</p>
                                    <p class="ListHeadEngTxt">Rating ID</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">عنوان نقاط وكيل التهديد</p>
                                    <p class="ListHeadEngTxt">Rating Title</p>
                                </th>
                                <th style="padding-right: 100px;">
                                    <p class="ListHeadArbTxt">وصف نقاط وكيل التهديد</p>
                                    <p class="ListHeadEngTxt">Rating Description</p>
                                </th>
                            </tr>
                            @foreach ($ratings as $data)
                                <tr>
                                    ...
                                    <td style="width: 180px;"><a
                                            href="/threat-agent-rating-table/{{ $data->threat_agent_rating_id }}">{{ $data->threat_agent_rating_id }}</a>
                                    </td>
                                    <td>{{ $data->threat_agent_rating_title }}</td>
                                    <td>{{ $data->threat_agent_rating_description }}</td>
                                </tr>
                            @endforeach
                        </table>
                    </div>
    ...
    <script>
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/threat-agent-rating/edit/${selectedRadio.value}`;
            } else {
