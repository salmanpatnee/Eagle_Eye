...
<body>
    ...
    <!-- CONTENT -->
    <div class="IndiTable">
        <form method="POST" action="{{ route('delete.controlAssFind') }}">
            @csrf
            @method('DELETE')
            ...
            <div class="ListTable">
                <table cellspacing="0">
                    ...
                    @foreach ($controlAssessmentFindings as $controlAssessmentFinding)
                        <tr>
                            <td><input type="checkbox" name="selecteddelete[]" value="{{ $controlAssessmentFinding->control_finding_id }}">
                            </td>
                            <td><a
                                    href="/control-assessment-finding-table/{{ $controlAssessmentFinding->control_finding_id }}">{{ $controlAssessmentFinding->control_finding_id }}</a>
                            </td>
                            <td>{{ $controlAssessmentFinding->control_finding_name }}</td>
                            <td>{{ $controlAssessmentFinding->control_implementation_status }}</td>
                        </tr>
                    @endforeach
                </table>
            </div>
