...
<body>
    ...
    <div class="IndiTable">
        <form method="POST" action="{{ route('evidences.delete') }}" id="deleteForm">
            @csrf
            @method('DELETE')
                            ...
                            <tr>
                                <td>
                                    <input type="radio" name="record" class="record"
                                        value="{{ $evidence->$primaryKey }}" required>
                                </td>
                                <td>{{ $loop->index + 1 }}</td>
                                <td>
                                    <a href="{{ route('evidences.show', $evidence->evidence_id) }}">
    ...
    <script>
        ...
        document.getElementById('btnUpdate').addEventListener('click', function(event) {
            ...
            if (selectedRadio) {
                window.location.href = `/evidences/edit/${selectedRadio.value}`;
            } else {
