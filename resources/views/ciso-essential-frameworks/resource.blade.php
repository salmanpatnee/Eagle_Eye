...
@section('title', 'Framework Resource')
@section('content')
    <div>
        <x-table.action-wrapper title="{{ $cisoEssentialFramework?->id ? 'Update' : 'New' }} Resource">
            <x-action.button label="View" route_name="ciso-essential-frameworks.index" />
        </x-table.action-wrapper>

        <form action="{{ route('ciso-essential-frameworks.index') }}">
            @csrf
            <input type="hidden" id="resourceable_id" value="{{ $cisoEssentialFramework->id }}">
            <input type="hidden" id="resourceable_type" value="App\Models\CisoEssentialFramework">
            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6">
                <x-form.grid-col>
                    <div>
                        <x-form.field label="Framework Title" name="title" required="true"
                            placeholder="Enter Framework Title" :value="$cisoEssentialFramework?->title" readonly />
                    </div>
                </x-form.grid-col>
    ...
    <script>
        ...
        function initFilePondUploader(inputId, resourceType, acceptedFileTypes = null) {
            const options = {
                server: {
                    process: {
                        url: '{{ route('ciso-essential-frameworks.resource.store') }}',
                        method: 'POST',
                        headers: {
